...
public class NamingConvention : IEntityTypeAddedConvention, IEntityTypeAnnotationChangedConvention, IPropertyAddedConvention, IForeignKeyOwnershipChangedConvention, IKeyAddedConvention, IForeignKeyAddedConvention, IForeignKeyPropertiesChangedConvention, IIndexAddedConvention, IEntityTypeBaseTypeChangedConvention, IModelFinalizingConvention
{
    private static readonly StoreObjectType[] _storeObjectTypes = [StoreObjectType.Table, StoreObjectType.View, StoreObjectType.Function, StoreObjectType.SqlQuery];

    private readonly Dictionary<Type, string> _sets;

    private readonly NameConverter _nameConverter;

    public NamingConvention(IServiceProvider services, NameConverter converter)
    {
    ...
    public virtual void ProcessEntityTypeAdded(IConventionEntityTypeBuilder entityTypeBuilder, IConventionContext<IConventionEntityTypeBuilder> context)
    {
        ...
        {
            entityTypeBuilder.ToTable(_nameConverter.ConvertName(tableName), entityType.GetSchema());
        }

        if (entityType.GetViewNameConfigurationSource() == ConfigurationSource.Convention
            && entityType.GetViewName() is { } viewName)
        {
            entityTypeBuilder.ToView(_nameConverter.ConvertName(viewName), entityType.GetViewSchema());
        }
    }

    public void ProcessEntityTypeBaseTypeChanged(IConventionEntityTypeBuilder entityTypeBuilder, IConventionEntityType? newBaseType, IConventionEntityType? oldBaseType, IConventionContext<IConventionEntityType> context) => ProcessHierarchyChange(entityTypeBuilder);
    ...
    private void ProcessHierarchyChange(IConventionEntityTypeBuilder entityTypeBuilder)
    {
        ...
        string? GetDefaultTableName(IConventionEntityType entityType) => !entityType.HasSharedClrType && _sets.TryGetValue(entityType.ClrType, out var setName) ? setName : entityType.GetTableName();
    }

    public virtual void ProcessPropertyAdded(IConventionPropertyBuilder propertyBuilder, IConventionContext<IConventionPropertyBuilder> context) => RewriteColumnName(propertyBuilder);

    public void ProcessForeignKeyOwnershipChanged(IConventionForeignKeyBuilder relationshipBuilder, IConventionContext<bool?> context) => ProcessOwnershipChange(relationshipBuilder.Metadata, context);
        ...
        if (foreignKey.IsOwnership)
        {
            ...
            if (ownedEntityType.IsMappedToJson())
            {
                ...
                void ProcessJsonOwnedEntity(IConventionEntityType entityType, string? containerColumnName)
                {
                    ...
                    {
                        entityType.SetContainerColumnName(_nameConverter.ConvertName(containerColumnName));
                    }
                ...
                when StoreObjectIdentifier.Create(entityType, StoreObjectType.Table) is StoreObjectIdentifier tableIdentifier:
                {
                    ...
                    if (entityType.FindPrimaryKey() is IConventionKey primaryKey)
                    {
                        ...
                        if (entityType.FindRowInternalForeignKeys(tableIdentifier).FirstOrDefault() is null && !isTPT)
                        {
                            ...
                            {
                                primaryKey.Builder.HasName(_nameConverter.ConvertName(primaryKeyName));
                            }
                        }
                        else
                        {
                    ...
                    foreach (var foreignKey in entityType.GetDeclaredForeignKeys())
                    {
                        ...
                        {
                            foreignKey.Builder.HasConstraintName(_nameConverter.ConvertName(foreignKeyName));
                        }
                    }
                    ...
                    foreach (var index in entityType.GetDeclaredIndexes())
                    {
                        ...
                        {
                            index.Builder.HasDatabaseName(_nameConverter.ConvertName(indexName));
                        }
                    }
                        ...
                        && (string)annotation.Value != ownership.PrincipalEntityType.GetTableName())
                    {
                        ...
                        {
                            key.Builder.HasName(_nameConverter.ConvertName(keyName));
                        }
                    }
    ...
    public void ProcessForeignKeyAdded(IConventionForeignKeyBuilder relationshipBuilder, IConventionContext<IConventionForeignKeyBuilder> context)
    {
        ...
        {
            relationshipBuilder.HasConstraintName(_nameConverter.ConvertName(constraintName));
        }
    }
    ...
    public void ProcessKeyAdded(IConventionKeyBuilder keyBuilder, IConventionContext<IConventionKeyBuilder> context)
    {
        ...
        {
            keyBuilder.HasName(_nameConverter.ConvertName(keyName));
        }
    }
    ...
    public void ProcessIndexAdded(IConventionIndexBuilder indexBuilder, IConventionContext<IConventionIndexBuilder> context)
    {
        ...
        {
            indexBuilder.HasDatabaseName(_nameConverter.ConvertName(indexName));
        }
    }
    ...
    public void ProcessModelFinalizing(IConventionModelBuilder modelBuilder, IConventionContext<IConventionModelBuilder> context)
    {
        foreach (var entityType in modelBuilder.Metadata.GetEntityTypes())
        {
            ...
            foreach (var property in entityType.GetProperties())
            {
                ...
                {
                    property.Builder.HasColumnName(string.Concat(_nameConverter.ConvertName(entityType.ShortName()), columnName.AsSpan(entityType.ShortName().Length)));
                }
                ...
                if (property.Builder.CanSetColumnName(null))
                {
                    ...
                    {
                        property.Builder.HasColumnName(string.Concat(_nameConverter.ConvertName(shortName), columnName.AsSpan(shortName.Length)));
                    }
                }
                ...
                if (property.Builder.CanSetColumnName(null, storeObject.Value))
                {
                    ...
                    {
                        property.Builder.HasColumnName(string.Concat(_nameConverter.ConvertName(shortName), columnName.AsSpan(shortName.Length)), storeObject.Value);
                    }
                }
            }
        }
    ...
    private void RewriteColumnName(IConventionPropertyBuilder propertyBuilder)
    {
        ...
        {
            propertyBuilder.HasColumnName(_nameConverter.ConvertName(baseColumnName));
        }
        ...
        foreach (var storeObjectType in _storeObjectTypes)
        {
            ...
            {
                propertyBuilder.HasColumnName(_nameConverter.ConvertName(columnName), identifier.Value);
            }
        }
    }
}